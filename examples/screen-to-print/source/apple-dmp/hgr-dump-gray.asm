; ===========================================================================
; HGR GRAYSCALE (halftone) SCREEN DUMP  ->  Apple DMP via Parallel Interface
; Card.  B/W ribbon.
;
...
; fills + halftones each 8-line band, and streams genuine C. Itoh 8510 bit-image
; graphics (ESC n / ESC T / ESC G) one byte at a time to the Apple Parallel
; Interface Card's data latch.  Every data byte waits on the ACK-line ready bit
; first, so output is paced by the printer's own busy/ACK handshake exactly as
; on hardware -- no host framebuffer trick, no flooding.  The same program runs
; on a real Apple //e + Parallel card + Apple DMP and prints the identical page.
; Companion to common/hgr-plot.asm, which lays down the HGR test screen.
;
...
; wire format as imagewriteri/hgr-dump-gray.asm.  ONLY the transport differs: the
...
; half below -- NTSC classifier, gap fill, GRAY map, Bayer dither -- is identical
; to the IW1 HGR grey dump.
;
...
; re-assemble.
;
; Aspect + page are run-time selectable (same map as the colour dump):
