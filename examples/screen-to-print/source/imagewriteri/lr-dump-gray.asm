; ===========================================================================
; LO-RES (GR) SCREEN DUMP, GRAYSCALE  ->  ImageWriter I via Super Serial Card,
; fully relocatable 65C02.
;
; THE AUTHENTIC PATH: this 6502 code reads lo-res graphics RAM, transposes each
; colour row to a row of vertical dot columns, and streams genuine ImageWriter
; bit-image graphics (ESC n / ESC T / ESC G) one byte at a time to the Super
; Serial Card's ACIA. Every data byte waits on the transmitter-empty bit (TDRE)
; first, so output is paced at the real serial baud rate exactly as on hardware
; -- no host framebuffer trick, no flooding. The same program runs on a real
; Apple //e + SSC + ImageWriter I and prints the identical page. Companion to
; common/lr-plot.asm, which lays down the test screen this prints.
;
; --- GRAYSCALE COLOUR MODEL (ImageWriter I has no colour ribbon) -------------
...
; grey and the colour design prints as a faithful luminance map. This is the
; single-lo-res twin of imagewriteri/dlr-dump-gray.asm (same CDENS/BAYER), and
; the lo-res counterpart to the IW1 HGR/DHGR grey dumps; it replaces the older
; 1-bit binary lo-res dump that collapsed every colour 1..15 to solid black.
; (ESC K ribbon select is IW2-only and is deliberately NOT used here.)
;
...
;
; --- OUTPUT GEOMETRY ---------------------------------------------------------
; Each of the 48 colour rows prints as one 8-dot-high ESC G band. 40 cells * 13
; dots = 520 dots ~= 7.2" at 72 dpi, square dots, true 4:3 GR aspect. ESC T 16
...
; ($FBC1), known-RTS ($FF58), the stack page ($0100,X) and the SSC ACIA registers
; ($C0A8-$C0AB). The assembled bytes run unchanged at any load address; the
; default ORG below ($6000) is only where this build lands.
; ===========================================================================
