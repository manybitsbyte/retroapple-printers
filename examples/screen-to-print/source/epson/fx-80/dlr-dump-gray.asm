; ===========================================================================
; DOUBLE LO-RES (DLR) SCREEN DUMP, GRAYSCALE  ->  Epson FX-80 via Parallel
...
; streams genuine Epson ESC/P bit-image graphics (ESC A / ESC *) one byte at a
; time to the Apple Parallel Interface Card's data latch. Every data byte waits
; on the ACK-line ready bit first, so output is paced by the printer's own
; busy/ACK handshake exactly as on hardware -- no host framebuffer trick, no
; flooding. The same program runs on a real Apple //e + Parallel card + Epson
; FX-80 and prints the identical page. Companion to common/dlr-plot.asm, which
; lays down the DLR test screen this routine prints.
;
; The FX-80 speaks EPSON ESC/P, NOT the C. Itoh command core the DMP /
; ImageWriter share -- so the wire format DIFFERS from dmp/dlr-dump-gray.asm even
; though the transport (Parallel card, ACK handshake) is byte-for-byte the same,
; and the graphics half below -- AUXMOVE aux read, ROL4 aux phase, 7/6 dot pair
; split, dither -- is identical to that DMP double-lo-res dump. Two ESC/P
; specifics drive the wire-format rewrite (same two as epson/lr-dump-gray.asm):
...
;      against the 9-pin head's fixed 72 dpi vertical -- the exact 4:3 aspect the
;      C. Itoh dump got from ESC n. (Op manual 3-45; _starWidths[5]=dpi/72.)
...
;
; --- OUTPUT GEOMETRY ---------------------------------------------------------
; Each colour row prints as one 8-dot-high ESC * band. The 80 cells map to
...
; 72 dpi, square dots, the same true 4:3 aspect as a single-lo-res dump. ESC A 8
; sets an 8/72" = exactly-8-dot line pitch so the bands butt seamlessly.
;
; --- GRAYSCALE COLOUR MODEL (FX-80 is monochrome, black ribbon) --------------
; The FX-80 has no colour ribbon, so the 16 lo-res colours become 16 DISTINCT
; ink densities ordered by luminance (black = full ink .. white = bare paper) via
; the CDENS table. An ordered 4x4 Bayer dither (BAYER) turns each density into a
; dot field across the cell's column block, so all 16 colours read as their own
; grey and the colour design prints as a faithful luminance map. Same CDENS/BAYER
; as dmp/dlr-dump-gray.asm -- the grey model is transport-agnostic; only the wire
