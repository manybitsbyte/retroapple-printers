; ===========================================================================
; LO-RES (GR) SCREEN DUMP, GRAYSCALE  ->  Apple DMP via Parallel Interface Card,
; fully relocatable 65C02.
;
; THE AUTHENTIC PATH: this 6502 code reads lo-res graphics RAM, transposes each
; colour row to a row of vertical dot columns, and streams genuine C. Itoh 8510
; bit-image graphics (ESC n / ESC T / ESC G) one byte at a time to the Apple
...
; program runs on a real Apple //e + Parallel card + Apple DMP and prints the
; identical page. Companion to common/lr-plot.asm, which lays down the test
; screen this prints.
;
...
; wire format as imagewriteri/lr-dump-gray.asm. ONLY the transport differs: the
; ImageWriter rides the Super Serial Card's 6551 ACIA (poll TDRE, write TX reg);
; the DMP rides the Parallel card (poll ACK, write data latch). The graphics
; half below is identical to the IW1 lo-res grey dump.
;
...
; as imagewriteri/lr-dump-gray.asm. (ESC K colour-ribbon select is an ImageWriter
...
; as imagewriteri/lr-dump-gray.asm):
