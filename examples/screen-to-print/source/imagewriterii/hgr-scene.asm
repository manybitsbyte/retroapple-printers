...
; path) drawn straight into hi-res page 1 ($2000). Companion to the DHGR
; "Sierra" castle: this is its 280x192 HGR cousin, so the HGR colour and BW
; dumps print a recognisable scene instead of a contrived ramp.
;
; --- HGR COLOUR PACKING ------------------------------------------------------
...
;
; --- HGR LINE ADDRESSING -----------------------------------------------------
; Line L is NOT linear: base(L) = $2000 + (L&7)*$400 + ((L>>3)&7)*$80
;                                        + (L>>6)*$28.  CALCBASE computes it.
;
; Fixed ORG $6000 (consistent with dhgr-sierra). Draws into $2000, RTS when
; done; enables full-screen hi-res page 1 first so the scene shows as it builds.
; ===========================================================================
...
; --- entry: select full-screen hi-res page 1, then walk the record list ------
START   STA TXTCLR       ; graphics
